...
class Solution:
    def nearestExit(self, maze: list[list[str]], entrance: list[int]) -> int:
        ...
        distance = {tuple(entrance):0} # dictionary to hold distance of each node from entrance
        while queue:
            ...
            if (current_cell!=entrance) and ((current_row == maze_boundary[0]-1 or current_row==0) or (current_column==0 or current_column==maze_boundary[1]-1)):
                print('exit is at ',current_cell , 'with distance of ', distance[tuple(current_cell)])
                return distance[tuple(current_cell)]
            ...
            for neighbor in neighbors:
                ...
                if (neighbor not in visited) and (neighbor[0] in range(maze_boundary[0])) and (neighbor[1] in range(maze_boundary[1])):
                    if (maze[neighbor[0]][neighbor[1]]!="+"):
                            ...
                            distance[neighbor] = distance[tuple(current_cell)] + 1
        return -1
