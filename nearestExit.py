...
class Solution:
    def nearestExit(self, maze: list[list[str]], entrance: list[int]) -> int:
        ...
        def isExit(cell:tuple)->bool:
             # exits are when either row or column equal 0 or max value, we can't exit at entrance
            if (cell!=entrance) and ((cell[0] == maze_boundary[0]-1 or cell[0]==0) or (cell[1]==0 or cell[1]==maze_boundary[1]-1)):
            ...
            for neighbor in neighbors:
                ...
                if (neighbor not in visited) and (neighbor[0] in range(maze_boundary[0])) and (neighbor[1] in range(maze_boundary[1])):
                    if (maze[neighbor[0]][neighbor[1]]!="+"):
                            #if this neighbor is an exit, we can exit right now
                            if isExit(neighbor):
                                 print('exiting with distance of', current_distance)
